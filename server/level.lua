...
MySQL.ready(function()
    local data = MySQL.query.await('SELECT * FROM andrew_soldier')
...
local function save()
    local query = 'UPDATE andrew_soldier SET xp = ? WHERE user_identifier = ?'
    local parameters = {}
    local size = 0
...
local function createPlayer(identifier)
    ...
    MySQL.insert.await('INSERT INTO andrew_soldier (user_identifier, xp) VALUES(?, ?)', { identifier, levels[identifier] })
end

lib.callback.register('andrew_soldier:getLevel', function(source)
...
function AddPlayerLevel(player, amount)
    ...
    if math.floor(levels[identifier]) - level > 0 then
        TriggerClientEvent('andrew_soldier:showNotification', player.source, locale('unlocked_level'), 'success')
    end

    TriggerClientEvent('andrew_soldier:updateLevel', player.source, levels[identifier])
end
